...
public class CustomerDetailController {
    ...
    /**
     * Endpoint that creates a customer detail and returns it
     * @param customerDetail CustomerDetail to create
     * @return CustomerDetailResponseDTO
     */
    @PostMapping(value = "/add", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiOperation(
            value = "Returns the created customer detail",
            httpMethod = "POST",
            response = CustomerDetailResponseDTO.class
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<CustomerDetailResponseDTO> createNewCustomerDetail(
            @ApiParam(name = "customerDetail", required = true, value = "CustomerDetail")
            @Valid @RequestBody CustomerDetailDTO customerDetail) {

        CustomerDetailResponseDTO addressCreated = customerDetailService.createCustomerDetail(customerDetail);
    ...
    /**
     * Endpoint that deletes a customerDetail by its id
     * @param id CustomerDetail id
     * @return Void
     */
    @DeleteMapping(value = "/delete/{id}")
    @ApiOperation(
            value = "Empty response when deleting the customerDetail",
            httpMethod = "DELETE"
    )
    @ApiResponses(value = {
            @ApiResponse(
    ...
    public ResponseEntity<Void> deleteOneCustomerDetail(
            @ApiParam(name = "id", required = true, value = "Id", example = "1")
            @PathVariable("id") Long id) {
